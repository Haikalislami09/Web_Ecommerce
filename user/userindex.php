<?php 
    ...
    $queryuser = mysqli_query($mysqli, "SELECT * FROM tb_user WHERE id_user = '$id_user'");
...
<body>
    <header>
        ...
        <nav>
            <ul class="nav-links">
                ...
                <li><a href="order.php?id=<?= $id_user ?>">Order</a></li>
            </ul>
        </nav>
        <div class="search d-flex" id="search">
                        <div class="input-group my-2 ">
                            ...
                            <a href="favorite.php?id=<?= $rowuser['id_user'] ?>" class="text-decoration-none"><i class="fa-regular mt-2 mx-2 fa-heart text-dark"></i></a>
                                ...
                                <a href="profile.php?id=<?= $rowuser['id_user'] ?>" class="d-flex text-decoration-none text-light pt-2"><i class="fa-solid fa-user text-dark"></i></a>
                                <a href="" data-bs-toggle="modal" data-bs-target="#TopupModal<?= $rowuser['id_user'] ?>"><i class="fa-solid mx-2 mt-2 fa-wallet"></i></a>
                                <p class="mt-1">Rp.<?= number_format($rowuser['saldo'],0,',','.') ?></p>
                            </form>
                        </div>
                    </div>
                    <!-- Modal Top up-->
                    <div class="modal fade" id="TopupModal<?= $rowuser['id_user'] ?>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                    <div class="modal-dialog">
                        ...
                        <div class="modal-body">
                            <form action="../proses/addsaldo.php?id=<?= $rowuser['id_user'] ?>" method="post">
                                <div class="mb-3">
                                    <label for="topup" class="form-label">Top up</label>
                                    <select name="topup" class="form-control" id="">
                                        <option value="" readonly>Pilih jumlah top up...</option>
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" class="btn btn-dark">Top up</button>
                        </div>
                        </form>
                        </div>
                    </div>
        ...
        <?php 
            ...
            while ($rowbaru = mysqli_fetch_array($newproduct)) {?>
                <div class="new-img">
                    ...
                    <a href="#buyModal" data-bs-toggle="modal" data-bs-target="#buyModal<?= $rowbaru['id_barang'] ?>">
                                                Buy it
                                            </a>
                </div>
            <?php }?>
    ...
    <section class="filter-container">
        <form action="userindex.php#all-product" method="post">
            <div class="filter">
                <div class="outer">
                    <div class="inner">
                        <div class="icon">
        ...
        <?php
            ...
            while ($row = mysqli_fetch_array($result)) { ?>
                <div class="card mx-1" style="width: 20rem;">
                            <img src="../gambar/<?= $row['gambar'] ?>" class="card-img-top" width="500px" height="500px" alt="">
                        <div class="card-body">
                            <div class="nama">
                                <h2 class="card-tittle"><?= $row['nama_barang'] ?></h2>
                            </div>
                            <div class="descrip card-tittle"><p><?= $row['descrip'] ?></p></div>
                            <div class="harga"><p>Rp. <?= number_format($row['harga'], 0, ',', '.')?></p></div>
                            <div class="stok"><p><?= $row['stok'] ?></p></div>
                            <form method="post" action="../proses/addcart.php?action=add&id_barang=<?= $row['id_barang'] ?>">
